function [cpdag, best_score, cache] = yy_learn_struct_ges(data, nodesizes,seeddag, varargin)
%
...
best_score = yy_score_dag(data, seeddag, nodesizes);
cptt=0;
...
while ~done
    ...
    sold=best_score;
    nbrs = length(pdags);
    dags = yy_pdag_to_dag(pdags);
    scores = yy_score_dags(data, nodesizes, dags);
...
function scores = yy_score_dags(data, ns, dags)

scores = zeros(1,length(dags));
for t = 1:length(dags)
    scores(t) = yy_score_dag(data, dags{t}, ns);
end
