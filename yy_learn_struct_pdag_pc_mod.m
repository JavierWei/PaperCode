function [pdag,best_score, G] = yy_learn_struct_pdag_pc_mod(data, ns, k, alpha)
...
while ~done
 ...
 for i=1:length(X)
   ...
   if length(nbrs) >= ord & G(x,y) ~= 0
     ...
     for si=1:length(SS)
       S = SS{si};
       if yy_new_cond_indep_chisquare(x, y, S, data, alpha, ns)
         %if isempty(S)
...
pdag(pdag==-1) = 1;    % 我自己添加
best_score = yy_score_dag(data, pdag_to_dag(pdag), ns);
